...
export default function TabsComponent({ tabs }: TabsComponentProps) {
  ...
  return (
    <>
      <div>
        <div className='grid grid-cols-1 gap-1 px-5 py-2 md:gap-3 sm:grid-cols-3 glassEffect'>
          {tabs.map((tab, i) => (
            <button
              key={tab.title}
              className={clsx('py-2 rounded-md text-semibold border-blue-500 border-2', {
                'bg-transparent text-blue-500 hover:bg-blue-500 hover:text-white': selectedTab === i,
